...
void more_tests() {
  ...
  std::cout << "bool: " << ret_val.second;// << ", x: " << (*(ret_val.first)).x << ", y: " << (*(ret_val.first)).y << std::endl;
...
void student_tests() {
  ...
  //
  std::vector< std::pair<Point<int>,char> > pts;
  pts.push_back(std::make_pair(Point<int>(10,10), '3'));
  pts.push_back(std::make_pair(Point<int>(10,5), '3'));
  pts.push_back(std::make_pair(Point<int>(5,5), '1'));
  pts.push_back(std::make_pair(Point<int>(15,10), '2'));
  pts.push_back(std::make_pair(Point<int>(1,1), '0'));
  pts.push_back(std::make_pair(Point<int>(2,2), '4'));
  pts.push_back(std::make_pair(Point<int>(3,3), '6'));
  pts.push_back(std::make_pair(Point<int>(4,4), '9'));
  pts.push_back(std::make_pair(Point<int>(15,20), '8'));
  pts.push_back(std::make_pair(Point<int>(20,20), '5'));

  QuadTree<int,char> tests;
  assert (tests.size() == 0);
  // an empty tree has height == -1
  assert (tests.height() == -1); 
  // plot the structure with with these dimensions (width=40,height=20)
  std::cout << "\nan empty tree:" << std::endl;
  tests.plot(25,25);
  ...
  for (int i = 0; i < pts.size(); i++) {
    ...
    // add each point from the collection
    tests.insert(pts[i].first,pts[i].second);
    // verify the size (total # of pts in the tree)
    assert (tests.size() == i+1);
    ...
    /*if (i == 0) { 
      ...
      //tests.plot(25,25);
      // a tree with 1 node has height == 0
      assert (tests.height() == 0); 
      // check that the newly inserted element can be found
      QuadTree<int,std::string>::iterator itr = tests.find(10,10);
      assert (itr != tests.end());
      // read the label & coordinates from the iterator
      assert (itr.getLabel() == "3");
      // dereference the iterator to get the point
      const Point<int> &pt = *itr;
      assert (pt.x == 10);
      assert (pt.y == 10);
    } else if (i <= 4) {
      //std::cout << "\nafter inserting " << i+1 << " data pts:" << std::endl;
      //tests.plot(25,25);
      // the next 4 additions for this tests all happen at the
      // second level, tree has height = 1
      //assert (tests.height() == 1);
    } else if (i == 8) {
      std::cout << "\nafter inserting " << i+1 << " data pts:" << std::endl;
      tests.plot(40,20);
      assert (tests.height() == 2);
      // check for an element that exists
      QuadTree<int,int>::iterator itr = tests.find(4,7);
      assert (itr != tests.end());
      assert (itr.getLabel() == 'H');
      assert ((*itr).x == 4);
      assert ((*itr).y == 7);
      // check for a couple elements that aren't in the tree
      itr = tests.find(14,14);
      assert (itr == tests.end());
      itr = tests.find(15,18);
      assert (itr == tests.end());
      // another visualization of the tree structure
      // note: this is a pre-order traversal of the data (print the node, then recurse on each child)
      std::cout << "\na 'sideways' printing of the tree structure with 9 nodes:" << std::endl;
      tests.print_sideways();
    }*/
  }
  //tests.print_sideways();
