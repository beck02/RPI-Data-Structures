...
double FastMarchingMethod(Image<Color> &input, Image<DistancePixel> &distance_image) {
  ...
  for(int k = 0; k < black_pixels.size(); k++){
    //std::cout << "black pixel #" << k << ": x = " << black_pixels[k].getX() << ", y = " << black_pixels[k].getY() << std::endl;
    DistancePixel& curr_pix = distance_image.GetPixel(black_pixels[k].getX(), black_pixels[k].getY());
    //std::cout << "got current\n";
    for(int i = -1; i < 2; i++){
      for(int j = -1; j < 2; j++){
        if(i == 0 && j == 0) { continue; }
        int neighbor_x = curr_pix.getX() + i;
        int neighbor_y = curr_pix.getY() + j;
        if(neighbor_x >= 0 && neighbor_x < w && neighbor_y >= 0 && neighbor_y < h){
          DistancePixel& neighbor = distance_image.GetPixel(neighbor_x, neighbor_y);
          //std::cout << "got neighbor\n";
          double dist = 1;
          if(i != 0 && j != 0){ dist = sqrt(2); }
          if(neighbor.getValue() > dist + curr_pix.getValue()){
            neighbor.setValue(dist + curr_pix.getValue());
  ...
  //std::cout << "done with first neighbors\n";

  while(!queue.empty()){
    const DistancePixel* curr_pix_ptr = queue.top();
    queue.pop();
    for(int i = -1; i < 2; i++){
      for(int j = -1; j < 2; j++){
        if(i == 0 && j == 0) { continue; }
        int neighbor_x = curr_pix_ptr->getX() + i;
        int neighbor_y = curr_pix_ptr->getY() + j;
        if(neighbor_x >= 0 && neighbor_x < w && neighbor_y >= 0 && neighbor_y < h){
          DistancePixel* neighbor = &distance_image.GetPixel(neighbor_x, neighbor_y);
          double dist = 1;
          if(i != 0 && j != 0){ dist = sqrt(2); }
          if(neighbor->getValue() > dist + curr_pix_ptr->getValue()){
            neighbor->setValue(dist + curr_pix_ptr->getValue());
            answer = std::max(answer, neighbor->getValue());
            if (queue.in_heap(neighbor)){
              queue.update_position(neighbor);
            } else{
              queue.push(neighbor);
