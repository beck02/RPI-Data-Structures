...
Sudoku::Sudoku(int s) {
  ...
  int legvals[size];
  for(int i = 1; i <= size; ++i){ legvals[i-1] = i; }
  board = vector<vector<set<int> > >(size, vector<set<int> >(size, set<int>(legvals, legvals+size)));
...
int Sudoku::Propagate(int i, int j) {
  ...
  if(board[i][j].size() == 1){
    ...
    for(int k = 0; k < size; ++k){
      if(k != i){ changed += board[k][j].erase(val); }
      if(k != j){ changed += board[i][k].erase(val); }
    }
    int iquad = i/quadrant_size;
    int jquad = j/quadrant_size;
    int istart = iquad * quadrant_size;
    int jstart = jquad * quadrant_size;
    for(int i2 = istart; i2 < istart + quadrant_size; ++i2){
      for(int j2 = jstart; j2 < jstart + quadrant_size; ++j2){
        if( (i2 != i) && (j2 != j) ){ changed += board[i2][j2].erase(val); }
      }
    }
  }
  return changed;
