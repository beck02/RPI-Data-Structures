...
Plant::Plant(map<string, vector<string> > f) : Organism(f) {
	...
	itr = facts.find("lives_in:");
	if(itr != f.end()){
		if(itr->second.size() != 1 || itr->second[0] != "land"){ throw 0; }
	}
}
...
LaysEggs::LaysEggs(map<string, vector<string> > f) : Animal(f) {
	...
	if( itr != f.end() ){
		if(itr->second[0] == "false") { throw 0; }
	} else{
		throw 0;
	}
	itr = facts.find("adult_weight:");
	if(itr != f.end()){
		if(itr->second[0] != "<100lbs"){ throw 0; }
	}
}
...
Bird::Bird(map<string, vector<string> > f) : Animal(f), LaysEggs(f), Endothermic(f) {
	...
	if(itr != f.end()){
		for(int i = 0; i < itr->second.size(); ++i){
			string thing = itr->second[i];
			if( thing != "feathers" || thing != "bill" ){ throw 0; }
		}
	}
}
...
Trout::Trout(map<string, vector<string> > f) : Animal(f), Fish(f) {
	...
	itr = facts.find("eats:");
	if(itr != f.end()){ 
		if(itr->second.size() != 1 || itr->second[0] != "bugs"){ throw 0; }
	}
}
...
Tortoise::Tortoise(map<string, vector<string> > f) : Animal(f), Reptile(f) {
	...
	itr = facts.find("eats:");
	if(itr != f.end()){ 
		if(itr->second.size() != 1 || itr->second[0] != "lettuce"){ throw 0; }
	}
	itr = facts.find("has:");
	if(itr != f.end()){ 
		if(itr->second.size() != 1 || itr->second[0] != "scales"){ throw 0; }
	}
}
...
Penguin::Penguin(map<string, vector<string> > f) : Animal(f), Bird(f) {
	...
	if(itr != f.end()){
		for(int i = 0; i < itr->second.size(); ++i){
			string thing = itr->second[i];
			if( thing == "air" ){ throw 0; }
		}
	}
	itr = facts.find("eats:");
	if(itr != f.end()){ 
		if(itr->second.size() != 1 || itr->second[0] != "fish"){ throw 0; }
	}
}
...
Bat::Bat(map<string, vector<string> > f) : Animal(f), Mammal(f) {
	//cout << "making a bat" << endl;
	map<string, vector<string> >::const_iterator itr = facts.find("has:");
	if(itr != f.end()){
		//cout << itr->second.size() << endl;
		for(int i = 0; i < itr->second.size(); ++i){
			string thing = itr->second[i];
			if( thing != "wings" || thing != "teeth" ){ throw 0.1; }
		}
	}
	itr = facts.find("eats:");
	if(itr != f.end()){ 
...
Tiger::Tiger(map<string, vector<string> > f) : Animal(f), Mammal(f) {
	...
	if(itr != f.end()){ 
		for(int i = 0; i < itr->second.size(); ++i){
			string thing = itr->second[i];
			if( thing != "animals" || thing != "deer" ){ throw 0; }
		}
	}
	itr = facts.find("adult_weight:");
	if(itr != f.end()){
...
Rhinoceros::Rhinoceros(map<string, vector<string> > f) : Animal(f), Mammal(f) {
	map<string, vector<string> >::const_iterator itr = facts.find("adult_weight:");
	if(itr != f.end()){
		if(itr->second[0] != ">100lbs"){ throw 0; }
	}
	itr = facts.find("eats:");
	if(itr != f.end()){ 
		if(itr->second.size() != 1 || itr->second[0] != "grass"){ throw 0; }
	}
}
...
Platypus::Platypus(map<string, vector<string> > f) : Animal(f), Mammal(f), LaysEggs(f) {
	...
	if(itr != f.end()){
		for(int i = 0; i < itr->second.size(); ++i){
			string thing = itr->second[i];
			if( thing != "bill" || thing != "fur" ){ throw 0; }
		}
	}
	itr = facts.find("eats:");
	if(itr != f.end()){ 
