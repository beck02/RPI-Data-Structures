...
void Tile::rotate(){
  assert( num_roads != 4 );
  assert( rotation_ % 90 == 0 );
  std::string tmpN = north_;
  north_ = west_;
  west_ = south_;
  south_ = east_;
  east_ = tmpN;
  if( rotation_ == 0 ){ rotation_ = 90; }
  else if( rotation_ == 90 ){ rotation_ = 180; }
  else if( rotation_ == 180 ){ rotation_ = 270; }
  else if( rotation_ == 270 ){ rotation_ = 0; }
  prepare_ascii_art();
}
...
void Tile::printRow(std::ostream &ostr, int row) const {
  ...
  if (row == 0 ) {
    ostr << '+' << std::string(GLOBAL_TILE_SIZE-4,'-') << tileNum_ << '-' << '+';
  } else if( row == GLOBAL_TILE_SIZE-1 ){
...
void Tile::printTile(std::ostream &ostr) const {
  ostr << '+' << std::string(GLOBAL_TILE_SIZE-4,'-') << tileNum_ << '-' << '+' << std::endl;
  for(int row = 1; row < GLOBAL_TILE_SIZE-1; ++row){
    ostr << '|' << ascii_art[row-1] << '|' << std::endl;
  }
  ostr << '+' << std::string(GLOBAL_TILE_SIZE-2,'-') << '+' << std::endl;
}
...
bool operator==(const Tile &t1, const Tile &t2){
  return (t1.getNorth() == t2.getNorth()) && (t1.getEast() == t2.getEast()) && 
          (t1.getSouth() == t2.getSouth()) && (t1.getWest() == t2.getWest());
}
